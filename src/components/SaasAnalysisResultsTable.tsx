...
const SaasAnalysisResultsTable: React.FC<SaasAnalysisResultsTableProps> = ({
  ...
  const getValidityBadgeStyle = (validity: string) =>
    getThemeValidityColor(validity, theme);
  const getScoreColorStyle = (score: number) =>
    getThemeScoreColor(score, theme);

  const getRatingColorStyle = (rating: number) =>
    getThemeScoreColor(rating, theme);
  ...
  const renderCellContent = (
    ...
    switch (column.key) {
      ...
      case "innovationLevel":
        return (
          <Badge
            className={`${getInnovationLevelColor(
              value as string
            )} font-medium`}
          >
            {value as string}
    ...
    >
                            ...
                            {result.detailedAnalysis?.recommendations && (
                              <div>
                                ...
                                <ul className="space-y-1">
                                  {result.detailedAnalysis.recommendations.map(
                                    (rec, idx) => (
                                      <li
                                        key={idx}
      ...
      >
        ...
        <ul className="text-sm text-blue-800 space-y-1">
          <li>
            • Provide detailed descriptions of your SaaS idea for more accurate
            analysis
          </li>
          <li>• Include target market information and problem statements</li>
          <li>
            • Click the expand button (↓) to see detailed recommendations for
            each analysis
          </li>
